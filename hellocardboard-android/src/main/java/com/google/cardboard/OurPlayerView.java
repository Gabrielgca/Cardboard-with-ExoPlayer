 ...
 *
 * <p>A OurPlayerView can be customized by setting attributes (or calling corresponding methods),
 * overriding drawables, overriding the view's layout file, or by specifying a custom view layout
 * file.
 *
 * <h3>Attributes</h3>
 *
 * The following attributes can be set on a OurPlayerView when used in a layout XML file:
 *
 * <ul>
 ...
 *       also be set on a OurPlayerView, and will be propagated to the inflated {@link
 ...
 *
 * To customize the layout of OurPlayerView throughout your app, or just for certain configurations,
 ...
 * inflated for use by OurPlayerView. The view identifies and binds its children by looking for the
 * following ids:
 *
 * <ul>
 ...
 *       DefaultTimeBar} attributes set on the OurPlayerView will not be automatically propagated
 ...
 *
 * Defining your own {@code exo_player_view.xml} is useful to customize the layout of OurPlayerView
 ...
 * OurPlayerView. This will cause the specified layout to be inflated instead of {@code
 * exo_player_view.xml} for only the instance on which the attribute is set.
 */
public class OurPlayerView extends FrameLayout implements AdViewProvider {
    ...
    public OurPlayerView(Context context) {
        this(context, /* attrs= */ null);
    }

    public OurPlayerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, /* defStyleAttr= */ 0);
    }

    @SuppressWarnings({"nullness:argument.type.incompatible", "nullness:method.invocation.invalid"})
    public OurPlayerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.OurPlayerView, 0, 0);
            try {
                shutterColorSet = a.hasValue(R.styleable.OurPlayerView_shutter_background_color);
                shutterColor = a.getColor(R.styleable.OurPlayerView_shutter_background_color, shutterColor);
                playerLayoutId = a.getResourceId(R.styleable.OurPlayerView_player_layout_id, playerLayoutId);
                useArtwork = a.getBoolean(R.styleable.OurPlayerView_use_artwork, useArtwork);
                defaultArtworkId =
                        a.getResourceId(R.styleable.OurPlayerView_default_artwork, defaultArtworkId);
                useController = a.getBoolean(R.styleable.OurPlayerView_use_controller, useController);
                surfaceType = a.getInt(R.styleable.OurPlayerView_surface_type, surfaceType);
                resizeMode = a.getInt(R.styleable.OurPlayerView_resize_mode, resizeMode);
                controllerShowTimeoutMs =
                        a.getInt(R.styleable.OurPlayerView_show_timeout, controllerShowTimeoutMs);
                controllerHideOnTouch =
                        a.getBoolean(R.styleable.OurPlayerView_hide_on_touch, controllerHideOnTouch);
                controllerAutoShow = a.getBoolean(R.styleable.OurPlayerView_auto_show, controllerAutoShow);
                showBuffering = a.getInteger(R.styleable.OurPlayerView_show_buffering, showBuffering);
                keepContentOnPlayerReset =
                        a.getBoolean(
                                R.styleable.OurPlayerView_keep_content_on_player_reset, keepContentOnPlayerReset);
                controllerHideDuringAds =
                        a.getBoolean(R.styleable.OurPlayerView_hide_during_ads, controllerHideDuringAds);
            } finally {
                a.recycle();
            }
        }
        ...
        if (contentFrame != null && surfaceType != SURFACE_TYPE_NONE) {
            ...
            switch (surfaceType) {
                ...
                case SURFACE_TYPE_SPHERICAL_GL_SURFACE_VIEW:
                    surfaceView = new OurSphericalGLSurfaceView(context);
            ...
            // We don't want surfaceView to be clickable separately to the OurPlayerView itself, but we do
            // want to register as an OnClickListener so that surfaceView implementations can propagate
            // click events up to the OurPlayerView by calling their own performClick method.
    ...
    public static void switchTargetView(
            Player player, @Nullable OurPlayerView oldPlayerView, @Nullable OurPlayerView newPlayerView) {
     ...
     * {@link #switchTargetView(Player, OurPlayerView, OurPlayerView)} rather than this method. If you do
     ...
     *   <li>{@link OurSphericalGLSurfaceView} if {@code surface_type} is {@code
     ...
     * </ul>
     *
     * @return The {@link SurfaceView}, {@link TextureView}, {@link OurSphericalGLSurfaceView}, {@link
    ...
    private final class ComponentListener
        ...
        public void onTracksChanged(TrackGroupArray tracks, TrackSelectionArray selections) {
            ...
            Player player = Assertions.checkNotNull(OurPlayerView.this.player);
