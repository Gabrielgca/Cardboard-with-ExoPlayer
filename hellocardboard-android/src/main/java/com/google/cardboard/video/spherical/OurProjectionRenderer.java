 ...
 * thread when ready.
 */
/* package */ final class OurProjectionRenderer {
     ...
     */
    public static boolean isSupported(OurProjection projection) {
        OurProjection.Mesh leftMesh = projection.leftMesh;
        OurProjection.Mesh rightMesh = projection.rightMesh;
        return leftMesh.getSubMeshCount() == 1
                && leftMesh.getSubMesh(0).textureId == OurProjection.SubMesh.VIDEO_TEXTURE_ID
                && rightMesh.getSubMeshCount() == 1
                && rightMesh.getSubMesh(0).textureId == OurProjection.SubMesh.VIDEO_TEXTURE_ID;
    }
    ...
    /**
     * Sets a {@link OurProjection} to be used.
     *
     * @param projection Contains the projection data to be rendered.
     * @see #isSupported(OurProjection)
     */
    public void setProjection(OurProjection projection) {
     ...
     * @param mvpMatrix The Model View OurProjection matrix.
    ...
    /* package */ void draw(int textureId, float[] mvpMatrix, boolean rightEye) {
        ...
        GLES20.glVertexAttribPointer(
                positionHandle,
                OurProjection.POSITION_COORDS_PER_VERTEX,
                GLES20.GL_FLOAT,
                false,
                OurProjection.POSITION_COORDS_PER_VERTEX * C.BYTES_PER_FLOAT,
        ...
        GLES20.glVertexAttribPointer(
                texCoordsHandle,
                OurProjection.TEXTURE_COORDS_PER_VERTEX,
                GLES20.GL_FLOAT,
                false,
                OurProjection.TEXTURE_COORDS_PER_VERTEX * C.BYTES_PER_FLOAT,
    ...
    private static class MeshData {
        ...
        @OurProjection.DrawMode private final int drawMode;

        public MeshData(OurProjection.SubMesh subMesh) {
            ...
            switch (subMesh.mode) {
                case OurProjection.DRAW_MODE_TRIANGLES_STRIP:
                    drawMode = GLES20.GL_TRIANGLE_STRIP;
                    break;
                case OurProjection.DRAW_MODE_TRIANGLES_FAN:
                    drawMode = GLES20.GL_TRIANGLE_FAN;
                    break;
                case OurProjection.DRAW_MODE_TRIANGLES:
                default:
